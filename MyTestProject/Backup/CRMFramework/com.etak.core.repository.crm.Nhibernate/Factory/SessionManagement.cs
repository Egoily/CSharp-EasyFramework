    ...
    public class SessionManagement
    {
        ...
        private SessionManagement()
        {
            try
            {
                ...
                BuildSessionFactoryForConnectionString("FINANCE", "FINANCE");
                BuildSessionFactoryForConnectionString("USAGE", "USAGE");
                BuildSessionFactoryForConnectionString("WORKCDR", "WORKCDR");
            }
            catch (Exception ex)
            {
        ...
        private void BuildSessionFactoryForConnectionString(String connectionStringName, String sessionFactoryName)
        {
            ...
            foreach (var typeInAssembly in assemblyiesToLoad.SelectMany(x => x.GetTypes()))
            {
                ...
                if (typeof(BusinessOperation).IsAssignableFrom(typeInAssembly) && !typeInAssembly.ContainsGenericParameters)
                    autoMappedTypes.Add(typeof(DynamicBusinessOperationClassMap<>).MakeGenericType(typeInAssembly));
            }
